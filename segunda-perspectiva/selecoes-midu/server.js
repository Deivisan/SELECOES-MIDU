...
<body>
    ...
    // Other pages
    if (url === '/vagas' || url === '/portal' || url === '/empresas' || url === '/sobre' || url === '/admin') {
      res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
      const pageName = url.slice(1).charAt(0).toUpperCase() + url.slice(2);
      res.end(`<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${pageName} - Seleções Midu</title>
    <style>
...
<body>
    <div class="container">
        <div class="logo">Seleções Midu</div>
        <h1>${pageName}</h1>
        <p>Página funcionando com Next.js 16 + Bun!</p>
